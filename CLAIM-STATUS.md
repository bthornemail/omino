...
| Miquel [8,4,4] | implemented | `make test-algorithmic-laws` verifies the bounded extended parity law. |
...
| `.o` as source carrier | implemented | `make test-o-lifecycle` verifies the lifecycle document and `.o` example attestation markers. |
...
| Backplane connector profile | physical_profile | Pinout and signal-integrity targets documented; no fabricated board, timing closure, or EMC validation claimed. |
...
| eMMC-style boot envelope profile | implemented | `make test-boot-envelope` verifies the 64-byte flash envelope, canonical gauge pre-header, big-endian bootstrap frame extraction, BQF bounds, centroid detection, and secure-attestation rejection. |
| TCG lowering rules | implemented | `docs/TCG-BACKEND-SPEC.md` and `examples/tcg/` record the QEMU TCG target profile; `make test-tcg-backend-spec` performs static checks without building QEMU. |
...
| Single OMNION word axis resolution | implemented | `make test-algorithmic-laws` verifies carry-depth separation, `R[b,q,phi]` local fiber accounting, six start branches, local240 projection, and `18 * 4 * 60 = 4320` local incidences inside one OMNION word context. |
...
| Multimedia federated sync media bridge | implemented | `make test-media-bridge` verifies attestation-locked/nonzero carrier gating for streaming, render, capture, and transcode bridge words while rejecting unattested or out-of-band effects. |
...
| Octahedral face router backend | backend_optional | Verilog router and vectors added; `make octahedral-router-test` verifies selector-face routing when simulator tools are available. The selector law itself is mandatory. |
| Octahedral type-level face checks | implemented | GHC harness validates selector-to-face and face-to-interface assignments, including rejection of the incorrect `0xA080 -> Face4` mapping. |
| Canonical authority type-level checks | implemented | GHC harness validates authority/witness namespace separation and rejects cross-authority witness assignments. |
| Metatron incidence scribe backend | backend_optional | Verilog block and vectors added; `make metatron-scribe-test` verifies place stepping, carry, sector-gated `0x18` witness, and pairwise plane flags. The inscription law itself is mandatory. |
| Omnicron epistemic COBS-CONS/BQF layer | implemented | `make test-omnicron-epistemic` verifies `0x00` trapping, `0x00..0x1F`/`0x20..0x7F`/`0x80..0xAF`/`0xB0..0xFF` classification, local 240-chart containment, and BQF `Q(x,y)=60x^2+16xy+4y^2` evaluation. |
| Omnicron BQF resolver backend | backend_optional | Verilog block and vectors added; `make omnicron-bqf-test` verifies diagnostic energy, byte-band routing, high-nibble delineation, and null-boundary trap behavior. The BQF law itself is mandatory. |
| Fano 5040 slot scheduler backend | backend_optional | Verilog block and vectors added; `make fano-slot-test` verifies `fano7*720 + role3*240 + local240`, ceiling slot `5039`, and bounds-fault zeroing. The slot law itself is mandatory. |
| EAL meta-assembler backend | backend_optional | Verilog block and vectors added; `make meta-assembler-test` verifies packed opcode/slot words, centroid reporting, slot ceiling rejection, and 7-bit token rejection. The packing law itself is mandatory. |
| Backplane interlock monitor backend | backend_optional | Verilog block and vectors added; `make backplane-monitor-test` verifies XOR-`0x80` phase mirror, Tetragrammatron `120` sum check, active-low lockout, and Hamming/Tetragrammatron/phase fault priority. The logical interlock law itself is mandatory. |
| Pure meta-compiler reference core | implemented | GHC harness validates modeled Surface-to-Parsed and Resolved-to-Lowered passes, slot arithmetic, and expected compile-time rejections for invalid local240, slot5040, and assembler-token bounds. |
...
| Externalized proof complex bibliography | external_proof | `docs/EXTERNAL-PROOF-COMPLEX.md` links Omino to `bthornemail/axiomatic-sovereignty` at observed commit `12ad1d4f860c400c2b6304a68dd6c45469ed41d9`; proof checks run outside Omino and are not part of `make check`. |
