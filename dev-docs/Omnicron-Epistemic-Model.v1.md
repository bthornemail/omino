...
The bounded Section 37 implementation is now split into C and optional RTL artifacts:
...
The C layer enforces:

```text
0x00       -> preserved null boundary trap
0x01..0x1F -> control / separator / delta band
0x20..0x7F -> readable observer surface
0x80..0xAF -> lazy .o carrier band
0xB0..0xFF -> high-bit sparse lazy surface
```

The high-nibble delineation flag follows the `11 + 5` split:

```text
0x0..0xA -> declared local field
0xB..0xF -> delineation / escape / frame field
```

The place-value metric is implemented as:
